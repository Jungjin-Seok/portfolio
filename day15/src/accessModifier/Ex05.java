...
class Test5{
	...
	public Test5(String name, int age) {
		this.name = name;
		this.age = age;
		count++;//count += 1;	// 객체를 생성할 때 마다 count의 값을 1 증가시킨다
	}
	...
	// 멤버 필드와 같은 타입/이름의 갑승ㄹ 매개변수로 전달받아 필드에 대입하는 생성자
...
public class Ex05 {
	// tip
	// 이클립스에서 static 요소는 이탤릭(기울림꼴)이 적용된다
	// 이클립스에서 final 요소는 볼듳 (굵게)가 적용된다
...
//		Test5.setCount(1); // 클래스로 접근해도 마찬가지
		...
		// static 필드는 클래스에 속한 값이며, 서로 다른 객체가 같은 값을 고융한다
