...
class Test4{
	int n1 = 10;				// 일반 멤버 필드, 객체마다 다른 값을 가지고 변경할 수 있다
	static int n2 = 20;			// 정적 멤버 필드, 모든 객체가 값을 공유하고 변경할 수 있다		/클래스에 고정
	final int n3 = 30;			// 불변 멤버 필드, 객체마다 다른 값을 가지고 변경할 수 없다
	static final int N4 = 40;	// 정적 불변 멤버 필드, 모든 객체가 값을 공유하고 변경할 수 있다

	// 클래스를 제외한 모든 요소ㅛ는 첫글자를 소문자로 작성하지만
	// static final 필드는 예외적으로 모든 글자를 대문자로 작성한다
	// 단어와 단어의 합성인 경우는 각 단어를 구분하기 위한 방법으로 _(언더바)를 사용한다
	
	public static int getN2() {	// static 메서드는 static필드를 참조해야한다
...
public class Ex04 {
	public static void main(String[] args) {
		// final만 지정한 필드는 생성자의 활용에 따라 서로 다른 객체가 다른 값을 가질수도 있다
...
//		static 요소는 객체마다 서로 다른 값이 아니라, 클래스 하나에 고저오딘 값이므로
//		[객체.요소] 형식보다는 [클래스.요소] 형슥으로 접근하는것이 올바르다
