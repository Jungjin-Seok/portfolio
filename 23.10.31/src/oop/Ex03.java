...
class Stack2{ // 내가 만들 프로그램에 어떤 형태의 데이터가 등장하는가?
	...
	// Ex02.java 파일의 Stack 클래스의 내용을 가져오다
	
	// 스택의 데이터 구조를 정의하는 부분, 명사형 데이터, 변수
	int[] arr = new int [5];
	int top;
	
	// 스택에 포함시키고 싶은 기능(함수, 메서드)을 가져온다
	// Ex02.java의 main함수 위에 있는 함수를 가져와서 일부 수정한다
	void push(int data) {
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] == 0) {		// 빈칸을 찾았다면
				arr[i] = data;		// 데이터를 넣고
				top = i;				// 마지막으로 넣은 데이터의 위치( index)를 top에 저장
				return;
			}
		}		
	}
	...
	int pop() {
		...
		for(int i = top; i != -1; i--) {	// 배열을 top부터 거꾸로 순회하면서
...
public class Ex03 {
	public static void main(String[] args) {
		...
		// 데이터, 객체(object)에 내장된 기능을 실행하며서 프로그램을 진행한다
		stack.push(10);
		stack.push(20);
		stack.push(30);
