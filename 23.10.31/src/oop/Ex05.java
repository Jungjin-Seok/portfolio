...
class PhoneBook{	// 내가 만드는 자료형
					...
					// 클래스를 작성할 때는 하나의 객체를 기준으로 생각하면서 내용을 구성한다
	...
	// 생성자(new가 들어가는) : 객체를 생성할 때 실행되는 특수 함수이며, *[클래스의 이름과 같은 이름을 사용하는 함수]*
	...
	// 생성자를 오버로딩(중복정의)하여, 매개벼수를 전달하지 않아도 객체를 생성할 수 있게 한다
	PhoneBook(){}
...
public class Ex05 {
	public static void main(String[] args) {
		...
		// 객체를 생성한 직후에는 객체 내부 멤버필드의 값이 0 혹은 null로 초기화 되어 있다
		...
		//클래스도 자료형이므로, 같은 자료형의 여러 데이터를 묶어서 처리하는 배여롤 만들 수 있다
		...
		// bo1과 ob2는 객체 생성 이후 필드에 값을 대입해서 사용한다
		// 멤버 필드가 많고 대입할 내용이 많으면 코드가 길어지기 때문에
		// 객체 생서과 동시에 값을 지정하는 방법이 있다
