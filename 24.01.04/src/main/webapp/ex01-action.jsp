...
<%
	...
	// 큰 용량의 파일을 단일 객체로 전송할 수 없기 때문에, 여러가지 파트로 분활하여 전송한 후 재조립한다
	// form-data 를 여러 파트로 분할하여 전송하는 방식을 multipart/form-data라고 한다
	
	// 양식 데이터 전송 시 분할 하여 제출하고, 재조립하는 방식을 사용하기 때문에
...
<%
// MultipartRequest(request, 저장디렉토리(폴더), 최대전송크기, 글자인코딩, 중복처리정책);
// 기존의 내장객체 request와 몇가지 옵션을 추가로 전달받아서
// 파일을 전송받을수 있는 새로운 request를 생성하는 라이브러리 클래스
