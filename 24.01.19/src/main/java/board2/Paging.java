...
//------------------------------------------------------------------------------페이지 넘길때 [1],[2],[3]...
	...
	private Paging(int page, int boardCount) {				// boardCount에서 page내용만 불러오기
		...
		perPage = 10;										// 여기 값을 바꾸면 화면에 출려되는 게시글의 개수가 바뀐다!!					
		offset = (page -1) * perPage;						// 건너뛸 게시글의 수
		fetch = perPage;									// 그 다음 perPage의 수 만큼 화면에 출력
		
		// 하단 페이지 번호들을 링크로 출력할때 필요한 요소들
		pageCount = boardCount / perPage;					// 페이지가 총 몇개인지 파악한다
		pageCount += (boardCount % perPage != 0) ? 1 : 0;	// 나머지가 있으면 1페이지 추가한다
		section = (page - 1) / 10;							// 페이지의 구역 설정 (01, 02, 03)
		begin = section * 10 + 1;							// 시작 페이지
		end = begin + 9;									// 끝 페이지
		prev = section != 0;								// 0번 구역이라면 이전구역이 없다. 그 외에는 이전구역이 있다
		next = pageCount > end;								// 총페이지개수가 마지막페이지번호보다 크면 다음이 있다
															// 50(총페이지개수) > 50(마지막페이지번호)
		
		if(end > pageCount) {								// 마지막 페이지번호가 10단위로 계산된 값보다 작으면
