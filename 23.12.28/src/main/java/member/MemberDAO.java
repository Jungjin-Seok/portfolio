...
/*
  ...
  Connection 이후에는 이전 순서대로 [PreparedStatment],[ResultSet]등이 필요하다
 ...
 */
	...
	private MemberDAO() { // 생성자 외부 호출
		try {
			init = new InitialContext();								// context.xml 불러오기
			ds = (DataSource)init.lookup("java:comp/env/jdbc/oracle");	// context 내부 DataSource 뷸러오기
	...
	// 로그인  입력값(id, password)를 전달받아서 일치하는 계정 하나를 반환하는 함수
	...
	// 수정
	   ...
	   // 삭제  userId를 전달받아서 테이블에서 레코드를 삭제하는 함수 
