...
//				 = 0 은 사용불가	// 대상을 가리키는 형식의 참조 변수이지만
								// 지금 당장은 가리킬 대상이 없다
		...
		// 배열의 값은 지정하지 않으나, [배열의 길이를 지정]하여 배열을 생성
		// 이 때 배열의 각 값은 [자료형에 대응되는 0값으로 자동 초기화]가 이루어진다
		// 따라서 아래 배열의 값은 { 0, 0, 0, 0, 0,}이다
		int[] arr4 = new int[5];
		
		// 배열을 생성할 때는 길이 혹은 값중에서 하나의 방법으로만 지정한다.
		// 둘다 지정하는 것은 문법에 맞이 않는다
//		int[] arr5 = new int[5] {1, 2, 3, 2, 1};	// 문법 틀림
		
		System.out.println("arr1 : " + arr1); // null 은 가리킬 대상이 없다는 뜻이다
		...
		// java.util.Arrrays 클래스의 toString() 함수를 이용하여 출력할수있다
