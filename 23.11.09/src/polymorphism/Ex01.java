...
class Dog extends Animal{	// 오버라이딩 및 개별 고유 기능은 서브클래스에
	...
	@Override		// 슈퍼클래스의 공통기능을 상속받은 후 내용을 재정의함(서브클래스마다 달라지는 내용)
	void bark() {	
...
public class Ex01 {
	public static void main(String[] args) {
		...
		// ob1으로 참조하면 Cat 클래스에 작성된 내용까지 모두 참조할수 있다
		
//		a1.play(); // Animal 클래스에는 play() 메서드가 없다(Cat의 고유기능)
//		a2.tailing(); // Animal 클래스에는 tailing() 메서드가 없다(Cat의 고유기능)
	
		a1.bark();	// 슈퍼클래스 Animal에는 name + "..."으로 정의 되어 있으나
		a2.bark();	// 각 서브클래스의 오버라이딩 내용으로 실행된다

//=====================================================================================================		
		
		// upcasting : 서브클래스의 객체를 슈퍼클래스로 참조하는 형태(자료형 변환), [묶어서 일괄처리를 하기 위해서]
...
//			arr[i].play();	// Cat의 고유메서드 (실행안됨)
//			arr[i].tailing;	// Dog의 고유메서드 (실행 안됨)
			arr[i].bark();	// Animal의 공통메서드 (실행가능, 내용은 각자 다름)
