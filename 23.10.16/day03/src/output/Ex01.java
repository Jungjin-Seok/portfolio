...
public class Ex01 {
	public static void main(String[] args) {
		...
		// System.out.println() : 괄호안에 하나의 값을 받아서 출력한다
		// System.out.print() : 괄호안에 하나의 값을 받아서 출력한다. 줄을 바꾸지 않는다.
		// System.out.printf() : 괄호 안에 서식과 값을 차례대로 전달받아서 출력한다
		
		System.out.println("a" + "aa");    // 출력 결과물이 세로로 나옴
	                                // 연산자는 값을 만들어내는 기호이기 때문에
									// 괄호안의 계산이 먼저 진행되고
									// 계산의 결과값 하나가 출력되는 순서이다
		System.out.println("b");
		System.out.println("c");
		
		
		
		System.out.print("d");  // 출력 결과물이 가로로 나옴
								// 둘다 출력할 값의 자료형이 무엇이라도 출력할 수 있다
		System.out.print("e");
		System.out.print("f");
		...
		System.out.println(Integer.class); //소속 페키지 확인 하는 방법
		System.out.println();
		
		System.out.println("A");		// print(0) 는 출력 후 줄을 바꾸지 않는다
		System.out.println("\n");		// \n은 줄바꿈 문자 (개행문자, newLine, lineFeed)이다
		System.out.println("B");		// 따라서 print("\n")과 println()은 같은 역할이다
		System.out.println("\r");		// \r은 커서를 줄 왼쪽 끝으로 이동시킨다(CrrigeReturn)
		...
		System.out.println();               // 		문자열이 섞이면 계산을 수행하지 않는다
		
		//print와 println의 차이는 주로 반복문 내부에서 값을 연속으로 출력할 때 활용한다
		
		for(int i = 0; i < 5; i++ ) {		// i를 출력하고 한칸 띄어쓰기하고 줄은 바꾸지 않기
