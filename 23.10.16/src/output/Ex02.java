...
public class Ex02 {
	public static void main(String[] args) {
		...
		// 서식은 문자열이기 때문에, 별도의 변수에 저장해 두었다가 활용할 수도 있다
...
*/
		...
		System.out.printf("num : [%05d]\n", num);		// 5자리에서 부족하면 앞으로 0으로 채용
		System.out.printf("num : [%,d]\n", 1234000);		// 천단위 구분기호 콤마 적용
		System.out.println();
		
		// 실수의 서식 (c언어와 달리 double도 %1f를 사용하지 않고, %f만 사용한다), 부동소수점 방식의 실수
		double ad = 3.14;
		
		System.out.printf("%f\n", ad);		// 기본은 소수점 이하 6자리까지 출력
		System.out.printf("%.1f\n", ad);		//  소수점 이하 1자리까지 출력
		System.out.printf("%.3f\n", ad);		//  소수점 이하 3자리까지 출력
		System.out.printf("%.3f\n", 3.14f);		//  소수점 이하 3자리까지 출력
		System.out.printf("%5.2f\n", ad);		//	소수점 포함 전체 자릿수는 5자리
												// 이후 소수점은 2자리까지 출력
		System.out.printf("%.0f\n", ad);		//  소수점 이하 출력하지 않기(보이기는 정수 형식)
		System.out.println();
		...
		// 이스케이프 시퀀스로 해결하면 된다 (tab, 시작지점으로 부터 4칸 혹은 8칸씩 간격 배치)
