...
<%! 
	...
	private int cpu = ran.nextInt(100) + 1;				// 입력마다 값이 바뀌니까(유지되어야 한다)

// String test = request; // 선언부에서는 request 및 response에 접근 불가 // 멤버 필드에 접근 불가
%>
<%
	...
	int user;			 // 사용자 입력값은 매번 변경되기도 하고, request로 받아와야한다						
	String msg = "";
...
// 	}
	if(input != null) {
		...
		if(user == cpu) {
			msg = String.format("정답. %d회 만에 정답을 맞춤",count);
		}
		else if(user < cpu){
