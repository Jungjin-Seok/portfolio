...
public class Ex01 {
	...
	static int multiple(int n1, int n2) {
		...
		return answer;		// return 은 함수의 종료를 의미한다
							// 함수가 종료될때 호출된 자리로 값을 돌려준다
		
	}
	// 함수에서 정의한 값은 아래에서 사용할 수 없다 
	
	public static void main(String[] args) {
		// 0이 되지 못한 값은 정수 범위에서 0으로 간주한다
		int result = multiple(10, 20);	// 함수의 호출(call), 함수이름을 부르면서 값을 전달하여 실행
		...
		// 인자 : 함수 내용이 실해오디기 위해서 필요한 값. 결과의 원인이 되는 값
		// 매개변수 : 함수 정의에서 ()안에 지정하는 변수. 인자값을 받기 위한 변수이다
		// 반환자료형 : 함수는 실행졸료 후 특정 값을 만들어내는데 이값의 자료형을 반환자료형이라고 한다
